...
def create_anomaly_cnn(input_shape=input_shape, conv_layer_datas=conv_layer_datas):
    """
    ...
    def conv_block(outputs, kernel, dilation, strides, filters):
        outputs = Lambda(
            lambda x: tf.pad(x,
                             [[0, 0],
                              [dilation, dilation],
                              [dilation, dilation],
                              [0, 0]] if len(input_shape) == 3 else [[0, 0],
                                                                     [dilation, dilation],
                                                                     [dilation, dilation]],
                             'REFLECT'))(outputs)
        outputs = Conv2D(filters=filters, kernel_size=kernel, strides=strides, padding='valid', dilation_rate=dilation,
    ...
    outputs = Lambda(lambda x: tf.pad(x, [[0, 0],
                                          [1, 1],
                                          [1, 1],
                                          [0, 0]] if len(input_shape) == 3 else [[0, 0],
                                                                                 [1, 1],
